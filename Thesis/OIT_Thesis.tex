...
As stated in the previous sections, the piezo-electric device produces a change in length ($\Delta L$) as a function of the voltage across the device. In order to properly design the function generator, which will be driving the PZT, a Michelson Interferometer was constructed (see Figure~\ref{fig:michalson-interferometer}) to determine the strain coefficient of the material. 

\begin{figure}
\centering
	\includegraphics[width=3in]{./piezo-electric_device/michalson_interferometer.png}
	\caption[Michalson Interferometer]{Michelson Interferometer used for precisely measuring the change in distance of the piezoelectric device per voltage differential placed across it.}
	\label{fig:michalson-interferometer}
\end{figure}
...
\begin{equation}
d = \frac{m\lambda}{2} = 2.2148 \mu \text{m}
\end{equation}

Where $\lambda$ is the wavelength of the light (632.8 nm) and $m$ is the number of fringe changes observed. This resulted in the piezoelectric strain constant ($d_{33}$) equivilent to $73.8 \text{nm/V}. With a voltage scan from 0-30V will produce roughly 7 FSR when used in the sFPI. This PZT will be adequate for this application.
